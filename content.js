(async function() {
  try {
    // đọc part đã được popup.js gắn vào window
    const part = window.__udemyPart;

    const part1 = [
      "43351934","43352732","43352030","43352608","43353038","43353168",
      "43353426","43353560","43353642","43353718","43353760","43353798",
      "43353926","43353974","43354000","43354020","43354056","43354074",
      "43354094"
    ];
    ...
    const lectureIds = part === "part1" ? part1 : part2;
    if (!lectureIds) {
      console.error("Không có danh sách lecture cho part:", part);
      return;
    }
    ...
    if (!slugMatch) {
      console.log("Không tìm thấy courseSlug");
      return;
    }
    const courseSlug = slugMatch[1];

    // gọi API để lấy courseId
    const res = await fetch(`https://fpl.udemy.com/api-2.0/courses/${courseSlug}/?fields[course]=id`, {
      credentials: "include"
    });
    if (!res.ok) {
      console.error("Không lấy được courseId");
      return;
    }
    ...
    // duyệt từng lectureId
    for (let i = 0; i < lectureIds.length; i++) {
      const lectureId = lectureIds[i];
      const done = await fetch(`https://fpl.udemy.com/api-2.0/users/me/subscribed-courses/${courseId}/completed-lectures/`, {
        method: "POST",
        headers: {
      ...
      await new Promise(r => setTimeout(r, 500)); // delay nhỏ
    }

    alert(`Đã hoàn thành đánh dấu ${part}!`);
